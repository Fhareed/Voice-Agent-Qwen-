...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = VisRetNet( num_classes=10, embed_dims=[64, 128, 256, 512], depths=[2, 2, 8, 2], num_heads=[4, 4, 8, 16],init_values=[2, 2, 2, 2],
        heads_ranges=[4, 4, 6, 6],mlp_ratios=[3, 3, 3, 3], drop_path_rate=0.1,chunkwise_recurrents=[True, True, False, False], layerscales=[False, False, False, False]).to(device)
...
optimizer = SAM(model.parameters(), base_optimizer, rho=0.05, lr=0.0005)
num_epochs = 100
...
# ------------- ✅ TRAIN FUNCTION FOR SAM OPTIMIZER -------------
def train_one_epoch_sam(model, criterion, trainloader, optimizer, device, epoch):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
        ...
        if idx % print_freq == 0:
            for name, param in model.named_parameters():
                if param.grad is not None:
                    print(f"Epoch {epoch}, Batch {idx}, Grad norm [{name}]: {param.grad.norm().item():.4f}")
...
for epoch in range(num_epochs):
    train_loss, train_acc = train_one_epoch_sam(model, criterion, trainloader, optimizer, device, epoch)
